...
=============================================================================================================================
(Standard libraries are not described here.)
-----------------------------------------------------------------------------------------------------------------------------
...
for i in range(signs.shape[0]):
    ...
    if len(change) == 0:
        ...
        mushy.append(change[-1].item()+1)
pp.plotxy(change,'dimenstionless depth','Evolution of change Zone over time','Dimensionless Time')
mushy = np.array(mushy)
mushy = mushy.astype('float')
mushy = mushy*(EntlpyApp.Mesh.nl[1]-EntlpyApp.Mesh.nl[0])
poststart = time.perf_counter()
pp.plotxy(mushy,'dimenstionless depth','Evolution of Mushy Zone over time','Dimensionless Time')
...
if ip.test_case == 1:
    ...
    indices = np.arange(len(EntlpyApp.Hg[0,:]))
    fig, ax = plt.subplots()
