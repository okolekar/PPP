...
class Mesh():
    def __init__(self,s):
        ...
        self.J          = 0
        self.node_list  = []
        self.generate_node_list(self,s)
    ...
    def update_Jacobian(self,elem_no):
        if elem_no < ip.n:
            self.J = np.matmul(self.dphi,np.array(list(self.node_list[0][elem_no].values())).reshape(-1,1))
        else:
            self.J = np.matmul(self.dphi,np.array(list(self.node_list[1][elem_no].values())).reshape(-1,1))
