...
from test import Elemental_Subroutine
...
class FEA():
    def __init__(self,approach = None):
        if approach == None:
            ...
            self.list_s = []
            Initial     = SIC()
            self.s      = Initial.s_array[3]#1.3585838927376247e-05
            self.ds_dt  = Initial.ds_dt#0.00010426583981102262#
            self.t      = Initial.hist_t[3]#0.1303#
            self.histt  = [self.t]
...
#---------------------------------------------------------------------------------------------------------------------------#
        ...
        #while self.t<0.05:   
            ...
            Ns = E.Nsg
            bs = E.bsg
            ip.delt    = 0.000001#0.4*min(self.meshl.h**2,self.meshs.h**2)
...
#---------------------------------------------------------------------------------------------------------------------------#
        ...
        """pp.plotxy(self.list_s,"s","evolution of melt","time step")
        pp.Simultaneous_Plotter(Tl_History,'Liq Temp', Ts_History,'Solid Temp')
        pp.plotxy(Tl_History[0,:],"Temperature at first node","Time vs time","Time step")"""
        """global_nodes = np.row_stack((node_l_history,node_s_history)) 
        global_temp = np.row_stack((Tl_History,Ts_History))
        plt.plot(global_nodes,global_temp,'-o')
        plt.xlabel('Depth Dimentionless')
        plt.ylabel('total Temperature')
        plt.title('Temperature in solid and liquid Vs Depth ')
        plt.legend()
        plt.show()"""
        plt.plot(self.list_s,'-o')
        plt.xlabel('Time Step')
        plt.ylabel('Depth Dimensionless')
        plt.title('Numerical Evloution of the depth')
        plt.legend()
        plt.show()
        plt.plot(self.histt,self.list_s,'-o')
        plt.xlabel('Time')
        plt.ylabel('Depth Dimensionless')
        plt.title('Numerical Evloution of the depth')
        plt.legend()
        plt.show()
        """print("The Tl is here: - ")
        print(Tl_History)
        print("The Ts is here: - ")
        print(self.Ts)
        print(f"The s is evolved as follows {self.list_s}")"""
...
print(f"The time take is {end - start}")
print(f"The end time was {Stefan.t}")
print("\n")
fig, ax = plt.subplots()
