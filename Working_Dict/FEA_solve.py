...
5) Process is repeated untill the end time is achieved                                                                      
#############################################################################################################################
'''   
    def Stefan_problem_solver(self):
        ...
        for t in range(10):       #ip.t_end
            ...
            Ns = E.Nsg
            bs = E.bsg
            self.s = vu.s_update(self.s,self.Tl[-2],self.Ts[1], self.meshl.J,self.meshs.J)
...
#---------------------------------------------------------------------------------------------------------------------------#
            ...
            self.list_s.append(self.s)
        pp.Simultaneous_Plotter(Tl_History,'Liquid Temperature',Ts_History,'Solid Temperature')
...
pp.plotVerify(Stefan.list_s,"melt surface dist")
if ip.test_case == 1:
