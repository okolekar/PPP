...
#===========================================================================================================================#
    ...
    def Stefan_problem_solver(self):
        ...
        E.get_global_Stefan_param(self.meshl,self.meshs,self.ds_dt)
#there is a problem below. where np.eye is 2*2 matrix and the others are about 21*21 fix it
        self.Tl = np.linalg.inv(np.eye(2)+0.5*ip.delt*np.linalg.inv(E.Mlg)*E.Nlg)\
                  *((np.eye(2)-0.5*ip.delt*np.linalg.inv(Ml)*Nl)*self.Tl+\
                    ip.delt*(0.5*np.linalg.inv(E.Mlg)+0.5*np.linalg.inv(Ml))*bl)
        
        self.Ts = np.linalg.inv(np.eye(2)+0.5*ip.delt*np.linalg.inv(E.Msg)*E.Nsg)\
                  *((np.eye(2)-0.5*ip.delt*np.linalg.inv(Ms)*Ns)*self.Ts+\
                    ip.delt*(0.5*np.linalg.inv(E.Msg)*self.bsg+0.5*np.linalg.inv(Ms)*bs))
